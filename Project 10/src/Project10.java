...
public class Project10
{
	...
	public static void main(String[] args) throws FileNotFoundException
	{
		...
		String[] overSizeRows = new String[100];
		// Store the names of the columns in a perfect size array
		String[] perfectSizeColumns = {"Jan", "Feb", "Mar", "Apr"}; 
		...
		int lengthOverSize = overSizeRows.length; 
		int lengthPerfectSize = perfectSizeColumns.length; 
		double[][] twoDimension = new double[lengthOverSize][lengthPerfectSize]; 
		...
		int numberOfRowsUsed = readFile(overSizeRows, twoDimension, lengthPerfectSize, fileName); 
		// Call the performOperations method that handles most of the user interaction
		int numberOfRows = performOperations(overSizeRows, numberOfRowsUsed, perfectSizeColumns, twoDimension, keyboard); 
		// Call the writeFile method
		writeFile(overSizeRows, numberOfRows, lengthPerfectSize, twoDimension, fileName); 
		keyboard.close();
	}
	...
	public static int performOperations(String[] rows, int rowSize, String[] cols, double[][] data, Scanner input)
	{
		...
		while (menuChoice != 6)
		{
			...
			else if (menuChoice==2)
			{
				...
				total = findItemTotal(rows, rowSize, columnSize, data, item);
			...
			else if (menuChoice==3)  // grand total
			{
				...
				total = findGrandTotal(data, rowSize, columnSize);
			...
			else if (menuChoice==4)
			{
				...
				total = findRangeTotal(data, rowSize, columnSize, lower, upper);
	...
	{	
		// Check if the specified items are there
		int indexFoundItem = findLabelIndex(rows, cols.length, item);
		int indexFoundMonth = findLabelIndex(cols, rowSize, month); 
		
		// If found then add the amount from user to the array it was founded on
		if (indexFoundItem != -1 && indexFoundMonth != -1) {
			data[indexFoundItem][indexFoundMonth] += amount;
		}
		else {
			// Add a size since there is a new item/month
			rowSize++; 
//			The code would be used below if this was to return a array instead because the array can't be updated from an array
//			rows[rowSize] = item; 
//			data[rowSize][indexFoundMonth] = data[rowSize][indexFoundMonth] + amount;
		}
		
		return rowSize; // Return the size of Array
	}
	...
	{
		// Searches each row/column to determine if the specified label is found
	...
	{
		// Search for the total by determining the row
		double total = 0.0; 
		int result = findLabelIndex(cols, rowSize, month); // Searches for the column that has the specified month
		
			if (result != - 1) { // if the result is found then add everything in that column to get total
				for (int outer = 0; outer < rowSize; ++outer) {
						total += data[outer][result];
				}
			}
		return total; // total
	}
	...
	{ 
		// Find the grand total by reading all the rows and columns together to get sum of all the arrays
		double total = 0.0; 
		for (int outer = 0; outer < rowSize; ++outer) {
			for (int inner = 0; inner < colSize; ++inner) {
				total += data[outer][inner]; 
			}
		}
		return total; // total
	}
	...
	public static double findRangeTotal(double[][]data, int rowSize, int colSize, double lowerAmount, double upperAmount)
	{
		...
		for (int outer = 0; outer < rowSize; ++outer) {
			for (int inner = 0; inner < colSize; ++inner) {
				if (data[outer][inner] >= lowerAmount && data[outer][inner] <= upperAmount) { // Determine range and check to make sure it actually stays in the range specified
					total += data[outer][inner]; 
				}
			}
		}
	...
	public static double findItemTotal(String[] rows, int rowSize, int colSize, double[][]data, String item)
	{
		...
		double total = 0.0; 
		int result = findLabelIndex(rows, colSize, item); 
		
		if (result != -1) {
				for (int index = 0; index < colSize; ++index) {
					total += data[result][index]; 
			}
		}
		return total; // total
	}
